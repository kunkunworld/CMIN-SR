...
- `structure_aggregated_ols`: aggregate-increment structure-function baseline;
...
The Haar wavelet rows are dependency-free classical controls, not full
production WTMM or wavelet-leader packages. A full external WTMM/wavelet-leader
benchmark remains useful future work, but lightweight wavelet-style baselines
are now included.
