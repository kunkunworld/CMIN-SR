...
def main():
    ds=generate_raw_zeta_alignment_dataset(RawZetaAlignmentDatasetConfig(length=256,num_samples=16,seed=3))
    model=CMINSRZetaAlignedModel(); out=model(torch.tensor(ds["x"],dtype=torch.float32))
    loss=curvature_preserving_zeta_loss(out,torch.tensor(ds["process_code"],dtype=torch.long),torch.tensor(ds["zeta_target"],dtype=torch.float32),torch.tensor(ds["zeta_target_mask"],dtype=torch.float32),torch.tensor(ds["zeta_weight_by_q"],dtype=torch.float32),torch.tensor(ds["H_true"],dtype=torch.float32),torch.tensor(ds["lambda2_true"],dtype=torch.float32),torch.tensor(ds["target_tail_instability"],dtype=torch.float32))
    result={"status":"ok","loss":float(loss.total),"isfinite":bool(torch.isfinite(loss.total))}
    p=ROOT/"outputs"/"reports"/"curvature_preserving_zeta_losses_smoke_test.json"; p.write_text(json.dumps(result,indent=2),encoding="utf-8"); print(json.dumps(result,indent=2))
