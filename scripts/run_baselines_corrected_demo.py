...
def main() -> None:
    params = MRWParams(length=4096, dt=1.0, lambda2=0.12, L=512, sigma=1.0, H=0.72, seed=2026)
    ...
    sq = structure_functions_from_increments(sample["dx"], q_vals, sf_scales)
    zeta_sf = fit_zeta_from_structure_functions(sq, q_vals, sf_scales, fit_slice=slice(2, -2))
    fq = mfdfa(sample["dx"], q_vals, mfdfa_scales, order=1)
    hq = fit_hq_from_mfdfa(fq, q_vals, mfdfa_scales, fit_slice=slice(2, -2))
    ...
    output_path = output_dir / "baseline_corrected_demo.json"
