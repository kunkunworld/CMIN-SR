...
from mrw_inverse.losses import cmin_sr_v2_loss
from mrw_inverse.models import CMINSRv2Model


CHECKPOINT_PATH = ROOT / "checkpoints" / "cmin" / "cmin_sr_v2_synthetic.pt"
REPORT_DIR = ROOT / "outputs" / "reports" / "cmin_sr_v2_training"
TABLE_DIR = ROOT / "outputs" / "tables" / "cmin_sr_v2_training"
FIG_DIR = ROOT / "outputs" / "figures" / "cmin_sr_v2_training"


@dataclass
class SRV2TrainConfig:
...
def _evaluate(model: CMINSRv2Model, loaders: dict[int, DataLoader], cfg: SRV2TrainConfig):
    model.eval()
    metric_rows = []
    proc_rows = []
    preds_by_t = {}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train CMIN-SR v2 with monofractal calibration.")
    ...
    model = CMINSRv2Model().to(cfg.device)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        if val_mean < best_val:
            ...
            best_state = {
                ...
                "config": asdict(cfg),
                "model_name": "cmin_sr_v2_synthetic",
                "best_epoch": epoch,
            }
    ...
    if best_state is None:
        raise RuntimeError("CMIN-SR v2 training failed before checkpoint creation.")
    ...
    ax.legend()
    ax.set_title("CMIN-SR v2 training")
    ...
    for metric, file_name in [
        ...
        ("mean_lambda2_proj", "lambda2_proj_by_process.png"),
    ...
    summary_path = REPORT_DIR / "cmin_sr_v2_training_summary.md"
    ...
    (REPORT_DIR / "cmin_sr_v2_training_summary.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")
